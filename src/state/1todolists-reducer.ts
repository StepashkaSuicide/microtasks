...
const initialState: Array<TodolistType> = []
 ...
 const removeTodolistAC = (todolistId1: string) => {
    return {
        type: 'REMOVE-TODOLIST',
        payload: {todolistId: todolistId1}
    } as const
}
 const addTodolistAC = (newTodolistTitle: string) => {
 ...
 const changeTodolistTitleAC = (todolistId2: string, newTodolistTitle: string) => {
    return {
 ...
 const changeTodolistFilterAC = (newFilter: FilterValuesType, todolistId: string ) => {
    return {
        type: 'CHANGE-TODOLIST-FILTER',
        payload: {newFilter, todolistId: todolistId}
    } as const
}
 const editTodolistAC = (todolistId: string, newTitle: string) => {
    return {
        type: 'EDIT_TODOLIST',
...
const todolistsReducer = (state=initialState, action: tsarType):Array<TodolistType> => {
    switch (action.type) {
